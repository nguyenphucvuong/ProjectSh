            ...
            <View style={{ flexDirection: 'row', }}>
                ...
                <TouchableOpacity style={[styles.item, { backgroundColor }]} onPress={onPress3} >
                    <Image
                        style={styles.tinyLogo}
                        source={{
                            uri: item.imagePr.url3,
                        }} />
                </TouchableOpacity>
            </View>
            <View style={{ flexDirection: 'row', }}>
                <Text style={[styles.mau, { color: textColor }]}>Trắng</Text><View style={{ width: 80 }}></View>
                <Text style={[styles.mau, { color: textColor }]}>Đen</Text><View style={{ width: 80 }}></View>
                <Text style={[styles.mau, { color: textColor }]}>Đỏ</Text>
            </View>
...
const Detail = ({ props, route, navigation }) => {
    ...
    const read = async (id) => {
        const dbRef = ref(db);
        try {
            get(child(dbRef, `SanPham/${id}`)).then((snapshot) => {
                if (snapshot.exists()) {
                    let a = [];
                    a[0] = snapshot.val();
                    setData(a);
    ...
    const renderItem = ({ item }) => {
        return (
            <Item
                item={item}
                onPress={() => setSelectedId(item.imagePr.url1)}
                onPress2={() => setSelectedId(item.imagePr.url2)}
                onPress3={() => setSelectedId(item.imagePr.url3)}
